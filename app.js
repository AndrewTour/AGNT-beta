...
function parseMarketPulse(text=''){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    events.push({id:marketEventId(MARKET_PULSE_TYPES[typeKey],address,suburb),eventType:MARKET_PULSE_TYPES[typeKey],address,suburb,streetKey,receivedDate:todayKey(),createdAt:Date.now()});
  }
...
function renderMarketPulse(){
  ...
  const events=normaliseMarketPulseEvents(marketPulseEvents).sort((a,b)=>b.createdAt-a.createdAt),matched=events.reduce((n,event)=>n+marketMatches(event).length,0);
