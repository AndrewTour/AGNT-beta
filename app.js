...
function renderDayLog(){
  ...
  const wins=[];const todayData=dayData(k);if(Number(todayData.connects)>=targets.connects)wins.push(`${Number(todayData.connects)} connects target reached`);if(appointments)wins.push(`${appointments} appointment${appointments===1?'':'s'} booked`);const hotCalls=prospectInteractions.filter(x=>x.date===k&&x.type==='Call'&&x.marketEventId).length;if(hotCalls)wins.push(`${hotCalls} hot spotting call${hotCalls===1?'':'s'}`);const broadcasts=campaignHistory.filter(x=>dateKey(new Date(x.createdAt))===k);if(broadcasts.length)wins.push(`${broadcasts.length} broadcast${broadcasts.length===1?'':'s'} launched`);const winsCard=$('#todayWinsCard'),winsList=$('#todayWinsList');if(winsCard)winsCard.classList.toggle('hidden',!wins.length);if(winsList)winsList.innerHTML=wins.map(win=>`<span>✓ ${escapeHtml(win)}</span>`).join('');
...
function marketOpportunityScore(event,matches=marketMatches(event)){
  const rank={withdrawn:60,'just listed':52,listed:52,sold:42,'just sold':42,'price changed':32,'price change':32,'under offer':24,'auction result':20};
  const ageDays=Math.max(0,Math.floor((Date.now()-(Number(event.createdAt)||Date.now()))/86400000));
  return (rank[normalisePlace(event.eventType)]||15)+Math.min(30,matches.length*2)+Math.max(0,10-ageDays);
}
function marketOpportunityLabel(score){return score>=85?'Highest priority':score>=65?'Strong opportunity':score>=45?'Worth working':'Keep visible'}
function marketEventRecency(event){const days=Math.max(0,Math.floor((Date.now()-(Number(event.createdAt)||Date.now()))/86400000));return days===0?'Today':days===1?'Yesterday':`${days} days ago`}
function estimatedSessionMinutes(remaining){return remaining?Math.max(1,Math.round(remaining*1.5)):0}
function prospectQuality(p){
  let score=1;if(primaryProspectPhone(p))score++;if(p.temperature==='Hot'||p.temperature==='Warm')score++;if(!p.lastContact||Math.floor((parseKey(todayKey())-parseKey(p.lastContact))/86400000)>=30)score++;if(p.nextFollowUp)score++;
  const label=p.stage==='Seller'||p.sellingTimeframe?'Owner opportunity':p.lastContact?'Known contact':'New neighbour';return{score:Math.min(5,score),label};
}
function todayCallStreak(){const calls=prospectInteractions.filter(x=>x.date===todayKey()&&x.type==='Call').sort((a,b)=>a.at-b.at);if(!calls.length)return 0;let streak=1,best=1;for(let i=1;i<calls.length;i++){if((calls[i].at-calls[i-1].at)<=20*60*1000)streak++;else streak=1;best=Math.max(best,streak)}return streak}
function bestMarketInsight(){
  const grouped={};for(const event of marketPulseEvents){const key=event.eventType||'Market activity';const calls=prospectInteractions.filter(x=>x.marketEventId===event.id&&x.type==='Call');if(!grouped[key])grouped[key]={calls:0,connects:0};grouped[key].calls+=calls.length;grouped[key].connects+=calls.filter(x=>PROSPECT_CONNECTED_OUTCOMES.has(x.outcome)).length}
  const best=Object.entries(grouped).filter(([,v])=>v.calls>=2).sort((a,b)=>(b[1].connects/b[1].calls)-(a[1].connects/a[1].calls))[0];if(!best)return null;return{title:`${best[0]} is converting`,meta:`${best[1].connects} connects from ${best[1].calls} calls`};
}
...
function renderMarketPulse(){
  ...
  const events=normaliseMarketPulseEvents(marketPulseEvents).sort((a,b)=>marketOpportunityScore(b)-marketOpportunityScore(a)||b.createdAt-a.createdAt),matched=events.reduce((n,event)=>n+marketMatches(event).length,0);
  ...
  list.innerHTML=events.length?events.map(event=>{const matches=marketMatches(event),progress=marketSessionProgress(event,matches),buttonLabel=progress.complete?'Session Complete':progress.active?'Active Session':'Start Session',buttonClass=progress.complete?'primary market-session-complete-btn':progress.active?'primary market-session-active-btn':'primary market-session-start-btn',score=marketOpportunityScore(event,matches),remaining=Math.max(0,progress.total-progress.workedIds.size),minutes=estimatedSessionMinutes(remaining);return`<article class="market-pulse-card${progress.complete?' market-session-complete-card':''}"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><div class="market-priority-line"><strong>${escapeHtml(marketOpportunityLabel(score))}</strong><span>${escapeHtml(marketEventRecency(event))}</span></div><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p>${[event.daysOnMarket,event.guide,event.price].filter(Boolean).length?`<div class="market-pulse-details">${event.daysOnMarket?`<span>${escapeHtml(event.daysOnMarket)}</span>`:''}${event.guide?`<span>${escapeHtml(event.guide)}</span>`:''}${event.price?`<span>${escapeHtml(event.price)}</span>`:''}</div>`:''}<div class="market-pulse-match"><strong>${matches.length}</strong><span>Neighbours Found</span></div>${matches.length?`<div class="market-session-progress-copy"><strong>${remaining} remaining</strong><span>${progress.complete?'Completed':minutes?`About ${minutes} min left`:'Ready to review'}</span></div><div class="market-session-metrics"><span><strong>${progress.called}/${progress.total}</strong> called</span><span><strong>${progress.sms}</strong> SMS</span><span><strong>${progress.connects}</strong> connects</span><span><strong>${progress.followUps}</strong> follow-up${progress.followUps===1?'':'s'}</span></div>`:''}<button class="${buttonClass}" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length&&!progress.complete?'':'disabled'}>${matches.length?buttonLabel:'No Matching Contacts'}</button></article>`}).join(''):'<div class="prospect-empty"><strong>No hot spotting opportunities yet</strong><small>Paste today’s email below, then tap Find Opportunities.</small></div>';
}
...
function launchBulkSmsTest(){const payload=bulkSmsTestPayload();if(!payload.users.length)return toast('Add a valid test mobile');if(!cleanText($('#bulkSmsTestMessage')?.value,2000))return toast('Add a test message');if(!confirm(`Open ${AGNT_BULK_SMS_SHORTCUT} with ${payload.users.length} test message${payload.users.length===1?'':'s'}?`))return;bulkSmsTestLaunches.unshift({id:prospectId(),name:payload.name,count:payload.users.length,createdAt:Date.now()});saveLocal();renderBulkSmsTestHistory();window.location.href=agntBulkSmsShortcutUrl(payload)}
...
function renderProspecting(){
  ...
  const hotSpottingEvents=normaliseMarketPulseEvents(marketPulseEvents).filter(event=>marketMatches(event).length),hotSpottingReady=hotSpottingEvents.reduce((total,event)=>total+marketMatches(event).length,0),hotSpottingActive=hotSpottingEvents.some(event=>Boolean(event.sessionStartedAt)&&!event.sessionCompletedAt),hotSpottingComplete=Boolean(hotSpottingEvents.length)&&hotSpottingEvents.every(event=>Boolean(event.sessionCompletedAt)),hotSpottingButton=$('#openHotSpotting');if($('#hotSpottingReadyCount'))$('#hotSpottingReadyCount').textContent=hotSpottingReady;if($('#hotSpottingReadyLabel'))$('#hotSpottingReadyLabel').textContent=hotSpottingReady===1?'client ready':'clients ready';if(hotSpottingButton){hotSpottingButton.textContent=hotSpottingComplete?'Session Complete':hotSpottingActive?'Active Session':'Start Session';hotSpottingButton.classList.remove('market-session-start-btn','market-session-active-btn','market-session-complete-btn');hotSpottingButton.classList.add(hotSpottingComplete?'market-session-complete-btn':hotSpottingActive?'market-session-active-btn':'market-session-start-btn')}
  const pipelineMeta=$('#pipelineMissionMeta');if(pipelineMeta)pipelineMeta.textContent=pipelineSessionComplete?'All 50 worked':pipelineSessionActive?`${remainingPipeline.length} remaining · active session`:`${remainingPipeline.length} ready to call`;
  const activeStreets=hotSpottingEvents.filter(e=>e.sessionStartedAt&&!e.sessionCompletedAt).length,completeStreets=hotSpottingEvents.filter(e=>e.sessionCompletedAt).length,hotMeta=$('#hotSpottingMissionMeta');if(hotMeta)hotMeta.textContent=hotSpottingEvents.length?`${activeStreets} active · ${completeStreets} complete`:'No streets ready';
  const broadcastMeta=$('#broadcastMissionMeta');if(broadcastMeta)broadcastMeta.textContent=selectedBroadcastType?`${BROADCAST_TYPES[selectedBroadcastType]?.label||'Campaign'} draft ready`:'Choose a campaign type';
  const streak=todayCallStreak(),streakEl=$('#prospectorCallStreak'),streakMeta=$('#prospectorCallStreakMeta');if(streakEl)streakEl.textContent=streak;if(streakMeta)streakMeta.textContent=streak?`${streak} consecutive call${streak===1?'':'s'} in the current block`:'Start the next call';
  const insight=bestMarketInsight(),insightTitle=$('#prospectorInsightTitle'),insightMeta=$('#prospectorInsightMeta');if(insightTitle)insightTitle.textContent=insight?.title||'Build momentum';if(insightMeta)insightMeta.textContent=insight?.meta||'Your activity will reveal the best opportunities.';
...
function showProspectingSession(){if(!prospectSessionActive)return closeProspectDetail();if(prospectSection!=='today')return;$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingSession').classList.remove('hidden');while(prospectSessionIndex<prospectSessionIds.length){const current=prospectById(prospectSessionIds[prospectSessionIndex]);if(current&&!current.archived)break;prospectSessionIndex++}saveProspectingSessionState();if(prospectSessionIndex>=prospectSessionIds.length){$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End Session</button><span>${prospectSessionIds.length} completed</span></div><section class="prospect-session-card glass prospect-session-complete"><span class="prospect-avatar session-avatar">✓</span><h2>Queue complete</h2><p>You’ve worked through every contact in this session.</p><button class="primary" type="button" data-complete-market-session>Review & End Session</button></section>`;return}const id=prospectSessionIds[prospectSessionIndex],p=prospectById(id);const phone=primaryProspectPhone(p),tel=prospectTel(p),quality=prospectQuality(p),remaining=Math.max(0,prospectSessionIds.length-prospectSessionIndex),minutes=estimatedSessionMinutes(remaining);$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End Session</button><span>${prospectSessionIndex+1} of ${prospectSessionIds.length} · about ${minutes} min left</span></div>${prospectSessionContextStrip()}<section class="prospect-session-card glass"><span class="prospect-avatar session-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span>${escapeHtml(p.stage)} · ${p.temperature}</span><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||p.phone||'No contact detail added')}</p><div class="prospect-session-context"><div><span>LAST CONTACT</span><strong>${p.lastContact?fmtDate(p.lastContact):'Never'}</strong></div><div><span>NEXT FOLLOW-UP</span><strong>${p.nextFollowUp?fmtDate(p.nextFollowUp):'Not set'}</strong></div></div><div class="prospect-quality-strip"><span>CONTACT QUALITY</span><strong>${'★'.repeat(quality.score)}${'☆'.repeat(5-quality.score)}</strong><small>${escapeHtml(quality.label)}</small></div>${p.notes?`<blockquote>${escapeHtml(p.notes)}</blockquote>`:''}<div class="prospect-session-actions"><a class="primary ${phone?'':'disabled'}" href="${tel}" data-prospect-call="${p.id}" data-call-from-session="1">Call ${escapeHtml(p.name.split(' ')[0])}</a><button class="primary hotspot-sms-btn ${phone?'':'disabled'}" type="button" data-session-sms="${p.id}" ${phone?'':'disabled'}>SMS ${escapeHtml(hotSpotSmsFirstName(p))}</button></div><button class="secondary" type="button" data-session-log="${p.id}">Log Outcome</button><button class="text-btn" type="button" data-session-skip>Skip for now</button></section>`}

function endProspectingSession({completeMarketSession=false}={}){
  if(!prospectSessionActive)return closeProspectDetail();
  const stats={...prospectSessionStats},marketEventId=cleanText(prospectSessionContext?.eventId,160),returnToHotSpotting=Boolean(marketEventId),marketEvent=marketPulseEvents.find(e=>e.id===marketEventId),finalProgress=marketEvent?marketSessionProgress(marketEvent,marketMatches(marketEvent)):null;
  ...
  $('#prospectDetail').classList.add('hidden');
  $('#prospectingDashboard').classList.remove('hidden');
  setProspectorSection(returnToHotSpotting?'market':'today');
  renderProspecting();
  if(returnToHotSpotting)requestAnimationFrame(()=>{const view=$('#prospectingView');if(view)view.scrollTop=0});
  const overlay=document.createElement('div');overlay.className='prospect-session-review-overlay';overlay.innerHTML=`<section class="prospect-session-review glass" role="dialog" aria-modal="true" aria-label="Session review"><span class="eyebrow">SESSION REVIEW</span><h2>Strong work.</h2><p>Here’s what you completed.</p><div class="prospect-session-review-grid">${finalProgress?`<div><strong>${finalProgress.total}</strong><span>Neighbours</span></div><div><strong>${finalProgress.called}</strong><span>Calls</span></div><div><strong>${finalProgress.sms}</strong><span>SMS</span></div><div><strong>${finalProgress.connects}</strong><span>Connects</span></div>`:`<div><strong>${stats.calls}</strong><span>Calls</span></div><div><strong>${Number(stats.sms)||0}</strong><span>SMS</span></div><div><strong>${stats.connects}</strong><span>Connects</span></div>`}<div><strong>${stats.temperate}</strong><span>Warm / Hot</span></div><div><strong>${stats.appointments}</strong><span>Appointments</span></div>${finalProgress?`<div><strong>${Math.max(0,finalProgress.workedIds.size-finalProgress.called-finalProgress.sms)}</strong><span>Skipped</span></div><div><strong>${finalProgress.followUps}</strong><span>Follow-ups</span></div>`:''}</div><button class="primary" type="button" data-close-session-review>Done</button></section>`;document.body.append(overlay);overlay.querySelector('[data-close-session-review]').onclick=()=>overlay.remove();
}
...
function renderKnockingHotSpotting(){
  ...
  host.innerHTML=streets.map(item=>{const reasons=[...new Set(item.events.map(event=>cleanText(event.eventType,60)).filter(Boolean))],properties=item.events.map(event=>event.address).filter(Boolean),contacts=activeProspects().filter(p=>marketStreetKey(p.address,p.suburb)===item.key),prior=contacts.filter(p=>interactionsFor(p.id).length).length,latest=Math.max(...item.events.map(e=>Number(e.createdAt)||0));return `<article class="knocking-hotspot-item"><div><strong>${escapeHtml(item.street)}</strong><small>${escapeHtml(item.suburb)} · ${contacts.length} neighbours</small></div><div class="knocking-hotspot-reasons">${reasons.map(reason=>`<span class="market-event-tag event-${marketTypeClass(reason)}">${escapeHtml(reason)}</span>`).join('')}</div><p>${escapeHtml(properties.slice(0,2).join(' · '))}${properties.length>2?` · +${properties.length-2} more`:''}</p><ul><li>${escapeHtml(marketEventRecency({createdAt:latest}))} market activity</li><li>${prior} previous conversation${prior===1?'':'s'} on this street</li><li>${contacts.length?'Strong database coverage':'New area to build'}</li></ul></article>`}).join('');
}
