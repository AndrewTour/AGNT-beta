...
function parseMarketPulse(text=''){
  ...
  for(let i=0;i<lines.length;i++){
    ...
    const peopleAndAgency=priceIndex>=0?afterAddress.slice(priceIndex+1):[];const agency=peopleAndAgency.length?peopleAndAgency.at(-1):'';const agents=peopleAndAgency.slice(0,-1).filter(line=>!/^(?:property image|logo)$/i.test(line));
