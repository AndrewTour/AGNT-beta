...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('[data-session-back]')){$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingDashboard').classList.remove('hidden');if(cleanText(prospectSessionContext?.eventId,160))setProspectorSection('market');renderProspecting();return}
