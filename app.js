...
function pageHeaderState(id=activeViewId()){
  ...
  if(id==='prospectingView'){
    ...
    if(prospectSection==='market')return{title:'Market Pulse',subtitle:'Turn today’s property changes into focused calling sessions.'};if(prospectSection==='insights')return{title:label,subtitle:'See what is creating conversations and appointments.'};
...
$('#openMarketPulse')&&($('#openMarketPulse').onclick=()=>{setProspectorSection('market');renderProspecting()});
...
$('#clearMarketPulseEvents')&&($('#clearMarketPulseEvents').onclick=()=>{if(!marketPulseEvents.length)return;marketPulseEvents=[];saveLocal();renderMarketPulse();toast('Market Pulse cleared')});
