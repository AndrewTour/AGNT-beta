...
const AGNTListings=(()=>{
  ...
  function renderDetail(l){const date=chooseDefaultDate(l),plan=activityFor(l,date),entry=entryFor(l,date),score=execution(l),status=statusCopy(l),week=campaignWeek(l,date),over=overdueCount(l);
    $('#listingDetail').innerHTML=`
      ...
      <section class="listing-panel glass"><div class="listing-panel-head"><div><span>Next best action</span><h3>${esc(plan.title)}</h3></div><em>${esc(formatShortDate(date))}</em></div><p class="listing-objective">${esc(plan.objective)}</p><button type="button" class="listing-progress-control" data-jump-activity><span><small>Today’s progress</small><strong>${(entry.steps||[]).filter(Boolean).length} of ${plan.steps.length} actions complete</strong></span><b>${entry.done?'Completed':'Continue'} ›</b></button></section>
  ...
  function bind(){
    ...
    $('#listingsView')?.addEventListener('click',e=>{const add=e.target.closest('[data-add-listing]');if(add)return openListingForm();const open=e.target.closest('[data-open-listing]');if(open){activeId=open.dataset.openListing;selectedActivityDate='';render();return}if(e.target.closest('[data-listing-back]')){activeId='';selectedActivityDate='';render();return}const l=activeId&&find(activeId);if(!l)return;if(e.target.closest('[data-edit-listing]'))return openListingForm(l);const dateButton=e.target.closest('[data-activity-date]');if(dateButton){selectedActivityDate=dateButton.dataset.activityDate;render();return}if(e.target.closest('[data-jump-activity]')){document.querySelector('#listingDailyActivity')?.scrollIntoView({behavior:'smooth',block:'start'});return}if(e.target.closest('[data-listing-prompt]'))return openPrompt(l,chooseDefaultDate(l));if(e.target.closest('[data-complete-activity]')){const date=chooseDefaultDate(l),entry=entryFor(l,date),steps=activityFor(l,date).steps.map((_,i)=>entry.steps?.[i]||false);persistActivity(l,date,{done:!entry.done,doneAt:!entry.done?Date.now():null,steps});render();toast(entry.done?'Activity reopened':'Campaign activity complete')}});
