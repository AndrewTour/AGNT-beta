...
function buyerCardMetaText(p){const parts=[],config=buyerConfigText(p),budget=buyerMaximumBudget(p),suburb=buyerPrioritySuburbText(p);if(config!=='Configuration not set')parts.push(config);if(budget)parts.push(`Up to ${formatBuyerMoney(budget)}`);if(suburb)parts.push(suburb);return parts.join(' · ')||'Buyer criteria not set'}
