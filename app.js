...
function renderBuyerSessionHero(){const count=$('#buyerListReadyCount'),label=$('#buyerListReadyLabel'),state=$('#buyerListWorkState'),button=$('#openBuyerListSession');if(!count||!button)return;const remaining=buyerSessionRemaining();count.textContent=buyerSession.contacts.length?remaining:'PDF';label.textContent=buyerSession.contacts.length?`${remaining} buyer${remaining===1?'':'s'} remaining`:'Import an Agentbox call list';state.textContent=buyerSession.active?'Session ready to continue':buyerSession.contacts.length?'List imported and reviewed':'Name, mobile and address/suburb';button.textContent=buyerSession.active?'Continue Session':buyerSession.contacts.length?'Start Session':'Import PDF'}
async function extractBuyerPdf(file){const pdfjs=await import('https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.min.mjs');pdfjs.GlobalWorkerOptions.workerSrc='https://cdnjs.cloudflare.com/ajax/libs/pdf.js/4.10.38/pdf.worker.min.mjs';const pdf=await pdfjs.getDocument({data:await file.arrayBuffer()}).promise,contacts=[];for(let pageNo=1;pageNo<=pdf.numPages;pageNo++){const page=await pdf.getPage(pageNo),content=await page.getTextContent(),pageWidth=page.getViewport({scale:1}).width,lines=[];for(const item of content.items){const text=String(item.str||'').trim();if(!text)continue;const x=Number(item.transform?.[4])||0,y=Number(item.transform?.[5])||0;let line=lines.find(row=>Math.abs(row.y-y)<3);if(!line){line={y,cells:['','','']};lines.push(line)}const col=x<pageWidth*.36?0:x<pageWidth*.67?1:2;line.cells[col]+=(line.cells[col]?' ':'')+text}lines.sort((a,b)=>b.y-a.y);let current=null;for(const line of lines){let[name,address,contact]=line.cells.map(v=>v.trim());if(!name&&/^(Printed by:|Print Date:|Basic Contact\/Call List|Name)$/i.test(address||contact))continue;if(/^(Printed by:|Print Date:|Basic Contact\/Call List|Name|Page \d+ of \d+)$/i.test(name))continue;if(name&&name!=='Address'&&name!=='Contact'&&!name.includes('mcgrath-wnwh.agentboxcrm.com.au')){if(current)contacts.push(current);current={id:`buyer_${Date.now().toString(36)}_${contacts.length}_${pageNo}`,name,address,contact,doNotSms:/do not sms/i.test(contact)};}else if(current){if(address)current.address+=(current.address?' ':'')+address;if(contact)current.contact+=(current.contact?' ':'')+contact}}if(current)contacts.push(current)}return contacts.map(c=>{const match=c.contact.match(/\[M\]:\s*(\+?61|0)[\d\s()-]{7,16}/i);return{id:c.id,name:c.name.replace(/\s+/g,' ').trim(),phone:match?normaliseDialNumber(match[0].replace(/^\[M\]:\s*/i,'')):'',address:c.address.replace(/\s+/g,' ').trim(),doNotSms:c.doNotSms}}).filter(c=>c.phone.replace(/\D/g,'').length>=9)}
...
$('#manualCallDone').onclick=()=>{const pending=readPendingManualCall();closeManualCallOutcome();if(pending?.source==='buyer-session')showBuyerSession()};
...
const AGNTListings=(()=>{
  ...
  function calendarDates(l){const start=mondayOf(addDays(todayKey(),-7));const dates=[];for(let i=0;i<21;i++){const k=addDays(start,i);if(dayNumber(k)!==0)dates.push(k)}return dates}
