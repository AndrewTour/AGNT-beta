...
function filteredBuyers(){const q=cleanText($('#prospectSearch')?.value||'',120).toLowerCase();return activeBuyerProspects().filter(p=>buyerMatchesFilters(p)).filter(p=>!q||[p.name,p.phone,p.email,p.address,p.buyerStage,p.temperature,p.buyerPropertyType,...(p.buyerPositionTags||[]),buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax),...(p.buyerSuburbs||[]),...(p.buyerFeatures||[])].join(' ').toLowerCase().includes(q)).sort((a,b)=>a.name.localeCompare(b.name,'en-AU',{sensitivity:'base'})||(Number(b.updatedAt)||0)-(Number(a.updatedAt)||0))}
...
function buyerPositionTagsMarkup(p={}){return buyerPositionTags(p).map(tag=>`<em class="buyer-position-tag">${escapeHtml(tag)}</em>`).join('')}
...
function buyerRequirementRows(p){const tags=buyerTagsForCard(p),position=buyerPositionTags(p);return`<div class="buyer-detail-requirements"><div><span>BUDGET</span><strong>${escapeHtml(buyerBudgetText(p.buyerBudgetMin,p.buyerBudgetMax))}</strong></div><div><span>CONFIGURATION</span><strong>${escapeHtml(buyerConfigText(p))}</strong></div>${position.length?`<div class="wide buyer-detail-position"><span>POSITION</span><strong>${escapeHtml(position.join(' · '))}</strong></div>`:''}<div class="wide"><span>SUBURBS</span><strong>${escapeHtml((p.buyerSuburbs||[]).join(' · ')||'Not set')}</strong></div><div class="wide"><span>PROPERTY</span><strong>${escapeHtml(tags.join(' · ')||'Requirements not set')}</strong></div></div>`}
