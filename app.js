...
function renderMarketPulse(){
  ...
  list.innerHTML=events.length?events.map(event=>{const matches=marketMatches(event),preview=matches.slice(0,3).map(p=>`<span>${escapeHtml(p.name)} · ${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb))}</span>`).join('');return`<article class="market-pulse-card"><div class="market-pulse-card-head"><span class="market-event-tag event-${marketTypeClass(event.eventType)}">${escapeHtml(event.eventType)}</span><button type="button" data-remove-market-event="${escapeHtml(event.id)}" aria-label="Remove event">×</button></div><h4>${escapeHtml(event.address)}</h4><p>${escapeHtml(event.suburb)}</p>${[event.daysOnMarket,event.guide,event.price].filter(Boolean).length?`<div class="market-pulse-details">${event.daysOnMarket?`<span>${escapeHtml(event.daysOnMarket)}</span>`:''}${event.guide?`<span>${escapeHtml(event.guide)}</span>`:''}${event.price?`<span>${escapeHtml(event.price)}</span>`:''}</div>`:''}<div class="market-pulse-match"><strong>${matches.length}</strong><span>same-street contact${matches.length===1?'':'s'} found</span></div>${preview?`<div class="market-pulse-preview">${preview}${matches.length>3?`<em>+${matches.length-3} more</em>`:''}</div>`:''}<button class="primary" type="button" data-start-market-session="${escapeHtml(event.id)}" ${matches.length?'':'disabled'}>${matches.length?'Create Calling Session':'No Matching Contacts'}</button></article>`}).join(''):'<div class="prospect-empty"><strong>No hot spotting opportunities yet</strong><small>Paste today’s email below, then tap Find Opportunities.</small></div>';
}
function importMarketPulse(){const input=$('#marketPulseInput'),parsed=parseMarketPulse(input?.value||'');if(!parsed.length){$('#marketPulseImportStatus').textContent='No supported property events were found. Paste the full email text, including event labels and addresses.';return toast('No market events found')}const existing=new Set(marketPulseEvents.map(x=>x.id)),fresh=parsed.filter(x=>!existing.has(x.id));marketPulseEvents=normaliseMarketPulseEvents([...marketPulseEvents,...fresh]);saveLocal();renderMarketPulse();$('#marketPulseImportStatus').textContent=fresh.length?`${fresh.length} new event${fresh.length===1?'':'s'} imported. ${parsed.length-fresh.length?`${parsed.length-fresh.length} duplicate${parsed.length-fresh.length===1?' was':'s were'} skipped.`:''}`:`All ${parsed.length} event${parsed.length===1?' was':'s were'} already imported.`;toast(fresh.length?'Hot Spotting imported':'No new events')}
function startMarketPulseSession(eventId){const event=marketPulseEvents.find(x=>x.id===eventId);if(!event)return;const ids=marketMatches(event).map(p=>p.id).filter(id=>!prospectContactedToday(id));if(!ids.length)return toast('No available contacts for this session');prospectSessionIds=ids;prospectSessionIndex=0;prospectSessionActive=true;prospectSessionStats={calls:0,connects:0,temperate:0,appointments:0};prospectSessionContext={eventId:event.id,eventType:event.eventType,address:event.address,suburb:event.suburb,daysOnMarket:event.daysOnMarket||'',guide:event.guide||'',price:event.price||''};saveProspectingSessionState();setProspectorSection('today');showProspectingSession();requestAnimationFrame(()=>requestAnimationFrame(()=>{const view=$('#prospectingView');if(view)view.scrollTop=0;window.scrollTo({top:0,behavior:'instant'})}));toast(`${event.eventType} session ready`)}
...
function renderProspecting(){
  ...
  $('#prospectingOverdue').textContent=overdue;$('#prospectingToday').textContent=due;$('#prospectingHot').textContent=hot;$('#prospectingDueCount').textContent=remainingPipeline.length;$('#prospectingDueLabel').textContent=remainingPipeline.length===1?'client ready':'clients ready';const hotSpottingReady=normaliseMarketPulseEvents(marketPulseEvents).reduce((total,event)=>total+marketMatches(event).length,0);if($('#hotSpottingReadyCount'))$('#hotSpottingReadyCount').textContent=hotSpottingReady;if($('#hotSpottingReadyLabel'))$('#hotSpottingReadyLabel').textContent=hotSpottingReady===1?'client ready':'clients ready';
...
function showProspectingSession(){if(!prospectSessionActive)return closeProspectDetail();if(prospectSection!=='today')return;$('#prospectingDashboard').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingSession').classList.remove('hidden');while(prospectSessionIndex<prospectSessionIds.length){const current=prospectById(prospectSessionIds[prospectSessionIndex]);if(current&&!current.archived)break;prospectSessionIndex++}saveProspectingSessionState();if(prospectSessionIndex>=prospectSessionIds.length){$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End Session</button><span>${prospectSessionIds.length} completed</span></div><section class="prospect-session-card glass prospect-session-complete"><span class="prospect-avatar session-avatar">✓</span><h2>Queue complete</h2><p>You’ve worked through every contact in this session.</p><button class="primary" type="button" data-end-session>Review & End Session</button></section>`;return}const id=prospectSessionIds[prospectSessionIndex],p=prospectById(id);const phone=primaryProspectPhone(p),tel=prospectTel(p);$('#prospectingSession').innerHTML=`<div class="prospect-session-head"><button type="button" data-end-session>End Session</button><span>${prospectSessionIndex+1} of ${prospectSessionIds.length}</span></div>${prospectSessionContextStrip()}<section class="prospect-session-card glass"><span class="prospect-avatar session-avatar">${escapeHtml(p.name.split(/\s+/).slice(0,2).map(x=>x[0]).join('').toUpperCase())}</span><span>${escapeHtml(p.stage)} · ${p.temperature}</span><h2>${escapeHtml(p.name)}</h2><p>${escapeHtml(formatProspectAddress(p.address||p.company,p.suburb)||p.phone||'No contact detail added')}</p><div class="prospect-session-context"><div><span>LAST CONTACT</span><strong>${p.lastContact?fmtDate(p.lastContact):'Never'}</strong></div><div><span>NEXT FOLLOW-UP</span><strong>${p.nextFollowUp?fmtDate(p.nextFollowUp):'Not set'}</strong></div></div>${p.notes?`<blockquote>${escapeHtml(p.notes)}</blockquote>`:''}<a class="primary ${phone?'':'disabled'}" href="${tel}" data-prospect-call="${p.id}" data-call-from-session="1">Call ${escapeHtml(p.name.split(' ')[0])}</a><button class="secondary" type="button" data-session-log="${p.id}">Log Outcome</button><button class="text-btn" type="button" data-session-skip>Skip for now</button></section>`}

function endProspectingSession(){
  if(!prospectSessionActive)return closeProspectDetail();
  const stats={...prospectSessionStats},returnToHotSpotting=Boolean(prospectSessionContext);
...
$('#clearMarketPulseEvents')&&($('#clearMarketPulseEvents').onclick=()=>{if(!marketPulseEvents.length)return;marketPulseEvents=[];saveLocal();renderMarketPulse();toast('Hot Spotting cleared')});
...
$('#prospectingView').onclick=async e=>{
  ...
  const removeMarket=e.target.closest('[data-remove-market-event]');if(removeMarket){marketPulseEvents=marketPulseEvents.filter(x=>x.id!==removeMarket.dataset.removeMarketEvent);saveLocal();renderMarketPulse();return}
