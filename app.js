...
function resetState(){days={};targets={...DEFAULTS};workDays=[...DEFAULT_WORK_DAYS];agentName='';calendarPreference='outlook';leaderboardEntries=[];marketPulseEvents=[];selectedDate=todayKey();appointmentDate=selectedDate}
...
function leaderboardAppointmentDetailsBetween(startKey,endKey){
  ...
  return details.sort((a,b)=>String(a.scheduledDate||'').localeCompare(String(b.scheduledDate||''))||String(a.time||'').localeCompare(String(b.time||''))||String(a.contactName||'').localeCompare(String(b.contactName||'')));
}
...
async function completePendingProspectAppointmentFlow(){
  ...
  try{await applyProspectingOutcomeMetrics(outcome,interactionId,{awaitCloud:!fromSession})}catch(err){console.error('Prospector metric save failed',err);toast('Appointment saved. Metrics are pending sync.')}
  prospectInteractions=prospectInteractions.map(x=>x.id===interactionId?{...x,metricsApplied:true}:x);
  try{await saveProspecting({render:false,awaitCloud:!fromSession})}catch(err){console.error('Prospecting log save failed',err);toast('The appointment is saved locally. Please check sync.')}
...
function showLeaderboardAgentSummary(agentUid){
  ...
  const cards=details?.length?details.map(item=>`<article class="leaderboard-agent-appointment"><span>${escapeHtml(item.type)}</span><div><strong>${escapeHtml(item.contactName||'Client not recorded')}</strong><small>${escapeHtml(item.address||'Address not recorded')}</small><time>${escapeHtml(leaderboardAppointmentDateTime(item))}</time></div></article>`).join(''):details===null&&knownTotal?`<div class="leaderboard-agent-summary-empty"><strong>${knownTotal} appointment${knownTotal===1?'':'s'} booked</strong><p>Details will appear after this agent receives the latest app update.</p></div>`:`<div class="leaderboard-agent-summary-empty"><strong>No appointments booked</strong><p>There are no MAP, LAP or BAP appointments recorded for this period.</p></div>`;
...
function parseMarketPulse(text=''){
  ...
  for(let i=0;i<lines.length;i++){
    const upper=lines[i].toUpperCase(),typeKey=Object.keys(MARKET_PULSE_TYPES).find(key=>upper===key||upper.startsWith(key+' ')||upper.includes(' '+key+' '));
    if(!typeKey)continue;
    let addressLine='';
    for(let j=i+1;j<Math.min(lines.length,i+6);j++){
      const candidate=lines[j];
      if(/\d/.test(candidate)&&candidate.includes(',')&&!/\b(?:bed|bath|car|land|days active|guide|price)\b/i.test(candidate)){addressLine=candidate;break}
    }
    if(!addressLine)continue;
    const {address,suburb}=splitMarketAddress(addressLine),streetKey=marketStreetKey(address,suburb);if(!streetKey||!suburb)continue;
    const detailLines=lines.slice(i+1,Math.min(lines.length,i+10));
    const detailValue=(labels=[])=>{for(const line of detailLines){const lower=line.toLowerCase();if(labels.some(label=>lower.includes(label))){const parts=line.split(/[:·|]/).map(x=>x.trim()).filter(Boolean);return cleanText(parts.length>1?parts.slice(1).join(' · '):line,120)}}return''};
    const daysOnMarket=detailValue(['days active','days on market','dom']);
    const guide=detailValue(['price guide','guide']);
    const price=detailValue(['asking price','listed price','sale price','price']);
    events.push({id:marketEventId(MARKET_PULSE_TYPES[typeKey],address,suburb),eventType:MARKET_PULSE_TYPES[typeKey],address,suburb,streetKey,receivedDate:todayKey(),createdAt:Date.now(),daysOnMarket,guide,price});
  }
...
function renderMarketPulse(){
  ...
  const events=normaliseMarketPulseEvents(marketPulseEvents).sort((a,b)=>{const ap=hotSpottingPriority(a,marketMatches(a).length),bp=hotSpottingPriority(b,marketMatches(b).length);return bp.score-ap.score||b.createdAt-a.createdAt}),matched=events.reduce((n,event)=>n+marketMatches(event).length,0);
...
function hotSpotSmsMessage(p){const c=prospectSessionContext||{},property=[c.address,c.suburb].filter(Boolean).join(', '),name=displayAgentName()||'Andrew Tour';return`Hi ${hotSpotSmsFirstName(p)},\n\n${name} from McGrath here. Just a quick heads up that ${property} ${hotSpotSmsEventPhrase(c)}${hotSpotSmsPricePhrase(c)}.\n\nIf you have any questions or would like to know what this means for your property, please don’t hesitate to let me know.\n\nThanks,\n${name} | McGrath`}
...
function showHotSpotSmsConfirmation(pending=loadHotSpotSmsPending()){if(!pending||!prospectSessionActive||pending.eventId!==cleanText(prospectSessionContext?.eventId,160))return;const p=prospectById(pending.prospectId);if(!p)return saveHotSpotSmsPending(null);const host=$('#prospectingSession');host.innerHTML=`<div class="prospect-session-head"><button type="button" data-sms-not-sent>‹ Back</button><span>Confirm SMS</span></div>${prospectSessionContextStrip()}<section class="prospect-session-card glass hotspot-sms-confirm"><span class="prospect-avatar session-avatar">✓</span><h2>Was the SMS sent?</h2><p>Confirm only after sending it in Messages.</p><button class="primary" type="button" data-sms-sent>SMS Sent</button><button class="secondary" type="button" data-sms-not-sent>Not Sent</button></section>`}
async function confirmHotSpotSmsSent(){const pending=loadHotSpotSmsPending();if(!pending)return;const p=prospectById(pending.prospectId);if(!p)return saveHotSpotSmsPending(null);const interactionId=prospectId(),at=Date.now();prospectInteractions.push({id:interactionId,prospectId:p.id,date:todayKey(),at,type:'SMS',outcome:'SMS sent',note:cleanText(pending.message,2000),nextFollowUp:'',marketEventId:cleanText(pending.eventId,160)});prospects=prospects.map(x=>x.id===p.id?normaliseProspect({...x,lastContact:todayKey(),updatedAt:at}):x);prospectSessionStats.sms=(Number(prospectSessionStats.sms)||0)+1;prospectSessionIndex++;saveHotSpotSmsPending(null);saveProspectingSessionState();try{await saveProspecting({render:false,awaitCloud:false})}catch(err){console.error('Hot Spotting SMS save failed',err);toast('SMS was saved locally. Please check sync.')}toast('SMS logged');showProspectingSession()}
function prospectSessionContextStrip(){if(!prospectSessionContext)return'';const c=prospectSessionContext,details=[c.eventType,c.daysOnMarket,c.guide,c.price].filter(Boolean);return`<aside class="market-session-context"><span>WHY YOU’RE CALLING</span><strong>${escapeHtml([c.address,c.suburb].filter(Boolean).join(', '))}</strong><small>${details.map(escapeHtml).join(' · ')}</small></aside>`}
...
$('#prospectingView').onclick=async e=>{
  ...
  if(e.target.closest('[data-session-back]')){$('#prospectingSession').classList.add('hidden');$('#prospectDetail').classList.add('hidden');$('#prospectingDashboard').classList.remove('hidden');return}
...
/* Listings campaign accountability calendar — isolated B19 addition */
const AGNTListings=(()=>{
  const DAY_PLAN={
    1:{short:'MON',title:'OFI buyer follow-up',objective:'Complete the first buyer follow-up round and give the vendor a brief position update.',steps:['Call every buyer from the latest open home','Establish interest, concerns and next action','Identify buyers worth progressing','Send a brief vendor update'],message:'brief'},
    2:{short:'TUE',title:'Second-round follow-up',objective:'Re-engage the strongest buyers and complete the full feedback report.',steps:['Complete the second call round','Ask engaged buyers for a clear position','Create second-inspection opportunities','Send the full feedback report'],message:'full'},
    3:{short:'WED',title:'Enquiry pulse',objective:'Check new enquiry and keep the vendor informed of the midweek campaign pulse.',steps:['Review all new enquiry since the weekend','Call any uncontacted enquiry','Revisit buyers requiring a next step','Send the vendor an enquiry pulse'],message:'pulse'},
    4:{short:'THU',title:'Third-round follow-up',objective:'Push active buyers forward and update the vendor on genuine movement.',steps:['Complete the third buyer call round','Ask serious buyers for an offer or next commitment','Confirm second inspections or contract activity','Update the vendor on the position'],message:'progress'},
    5:{short:'FRI',title:'Weekend preparation',objective:'Check the enquiry pipeline and set clear expectations with the vendor before Saturday.',steps:['Review new enquiry and buyer confirmations','Call buyers who may attend Saturday','Confirm the open-home plan','Send Saturday expectations to the vendor'],message:'expectations'},
    6:{short:'SAT',title:'Post-OFI confirmation',objective:'Close the loop immediately after the open home and confirm the follow-up plan.',steps:['Confirm attendee numbers with the vendor','Identify the strongest immediate interest','Set the buyer call-back window','Send a short post-OFI update'],message:'post-ofi'},
    0:{short:'SUN',title:'Campaign reset',objective:'Review the week and prepare the next campaign move.',steps:['Review incomplete campaign actions','Identify the strongest next opportunity','Confirm the coming week’s priority','Prepare the next vendor conversation'],message:'weekly'}
  };
  const DEFAULT_DAYS=[1,2,3,4,5,6];
  let activeId='',selectedActivityDate='';
  const key=()=>`${storagePrefix(uid)}listings-v2`;
  const legacyKey=()=>`${storagePrefix(uid)}listings-v1`;
  const readRaw=k=>{try{const v=JSON.parse(localStorage.getItem(k)||'[]');return Array.isArray(v)?v:[]}catch{return[]}};
  const read=()=>{const current=readRaw(key());if(current.length)return current;const legacy=readRaw(legacyKey());if(!legacy.length)return[];const migrated=legacy.map(x=>({id:x.id,address:x.address,vendorName:x.vendorName,startDate:x.startDate,method:x.method,auctionDate:x.auctionDate,notes:x.notes,status:x.stage==='exchange'?'sold':'live',logDays:DEFAULT_DAYS,activityLog:{},createdAt:x.createdAt,updatedAt:x.updatedAt}));try{localStorage.setItem(key(),JSON.stringify(migrated))}catch{}return migrated};
  const write=list=>{try{localStorage.setItem(key(),JSON.stringify(list));return true}catch{return false}};
  const esc=value=>escapeHtml(String(value??''));
  const clean=value=>String(value??'').trim();
  const addDays=(keyValue,amount)=>{const d=parseKey(keyValue);d.setDate(d.getDate()+amount);return dateKey(d)};
  const dayNumber=keyValue=>parseKey(keyValue).getDay();
  const mondayOf=keyValue=>{const d=parseKey(keyValue),day=d.getDay(),offset=day===0?-6:1-day;d.setDate(d.getDate()+offset);return dateKey(d)};
  const daysBetween=(from,to=todayKey())=>{if(!validDateKey(from)||!validDateKey(to))return 0;return Math.max(0,Math.floor((parseKey(to)-parseKey(from))/86400000)+1)};
  const campaignWeek=(l,keyValue=todayKey())=>Math.max(1,Math.floor((daysBetween(l.startDate,keyValue)-1)/7)+1);
  const formatShortDate=keyValue=>parseKey(keyValue).toLocaleDateString('en-AU',{day:'numeric',month:'short'});
  const formatLongDate=keyValue=>parseKey(keyValue).toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long'});
  const hydrate=raw=>({
    id:raw.id||uuid(),address:clean(raw.address),vendorName:clean(raw.vendorName),startDate:validDateKey(raw.startDate)?raw.startDate:todayKey(),method:raw.method||'Auction',auctionDate:validDateKey(raw.auctionDate)?raw.auctionDate:'',status:['live','under-offer','sold'].includes(raw.status)?raw.status:'live',logDays:Array.isArray(raw.logDays)&&raw.logDays.length?[...new Set(raw.logDays.map(Number).filter(n=>n>=0&&n<=6))]:DEFAULT_DAYS.slice(),activityLog:raw.activityLog&&typeof raw.activityLog==='object'?raw.activityLog:{},notes:clean(raw.notes),createdAt:Number(raw.createdAt)||Date.now(),updatedAt:Number(raw.updatedAt)||Date.now()
  });
  const all=()=>read().map(hydrate).filter(l=>l.status!=='sold').sort((a,b)=>b.updatedAt-a.updatedAt);
  const sold=()=>read().map(hydrate).filter(l=>l.status==='sold').sort((a,b)=>b.updatedAt-a.updatedAt);
  const saveListing=l=>{const list=read().map(hydrate),i=list.findIndex(x=>x.id===l.id),next=hydrate({...l,updatedAt:Date.now()});if(i>=0)list[i]=next;else list.unshift(next);write(list);return next};
  const find=id=>read().map(hydrate).find(x=>x.id===id);
  const isLogDay=(l,keyValue)=>l.logDays.includes(dayNumber(keyValue));
  const activityFor=(l,keyValue)=>DAY_PLAN[dayNumber(keyValue)];
  const entryFor=(l,keyValue)=>l.activityLog[keyValue]||{};
  const isDone=(l,keyValue)=>Boolean(entryFor(l,keyValue).done);
  const contextFor=(l,keyValue)=>{const week=campaignWeek(l,keyValue);if(week===1)return{label:'ESTABLISH THE CAMPAIGN',line:'Build a complete feedback picture, set the service rhythm and identify who can be moved forward.'};if(week===2)return{label:'BUILD COMPETITION',line:'Re-engage the best buyers, create second inspections and ask directly for a position.'};if(week===3)return{label:'CONVERT INTEREST',line:'Separate genuine buyers from observers and turn active interest into offers or clear decisions.'};return{label:'CREATE A DECISION',line:'Protect momentum, apply urgency and make sure the campaign does not drift without a clear next move.'}};
  const scheduledDates=(l,start=todayKey(),count=14)=>{const dates=[];for(let i=0;i<42&&dates.length<count;i++){const k=addDays(start,i);if(isLogDay(l,k)&&parseKey(k)>=parseKey(l.startDate))dates.push(k)}return dates};
  const nextDueDate=l=>{if(l.status==='sold')return'';const overdue=[];for(let i=0;i<35;i++){const k=addDays(todayKey(),-i);if(parseKey(k)<parseKey(l.startDate))break;if(isLogDay(l,k)&&!isDone(l,k))overdue.push(k)}if(overdue.length)return overdue.at(-1);return scheduledDates(l,todayKey(),1)[0]||''};
  const overdueCount=l=>{let n=0;for(let i=1;i<35;i++){const k=addDays(todayKey(),-i);if(parseKey(k)<parseKey(l.startDate))break;if(isLogDay(l,k)&&!isDone(l,k))n++}return n};
  const execution=l=>{const start=addDays(todayKey(),-13),eligible=[];for(let i=0;i<14;i++){const k=addDays(start,i);if(parseKey(k)>=parseKey(l.startDate)&&parseKey(k)<=parseKey(todayKey())&&isLogDay(l,k))eligible.push(k)}if(!eligible.length)return 100;return Math.round(eligible.filter(k=>isDone(l,k)).length/eligible.length*100)};
  const statusCopy=l=>{const overdue=overdueCount(l),score=execution(l);if(overdue)return['Action overdue',`${overdue} scheduled campaign ${overdue===1?'activity is':'activities are'} still incomplete.`];if(score===100)return['On track','Every scheduled campaign activity is complete.'];if(score>=75)return['Moving well','The campaign rhythm is being maintained.'];return['Needs attention','Complete today’s action to restore the campaign rhythm.']};
  const vendorFirst=l=>(l.vendorName||'there').split(/\s+/)[0];
  const messageFor=(l,keyValue)=>{const plan=activityFor(l,keyValue),week=campaignWeek(l,keyValue),first=vendorFirst(l),address=l.address,weekLine=week===1?'As we establish the campaign, our focus is getting a clear read on every buyer and making sure no opportunity is left sitting.':week===2?'This week our focus is progressing the strongest buyers, creating second inspections and asking for a clear position.':week===3?'At this stage of the campaign, the focus is converting genuine interest into an offer or a clear decision.':'The campaign is now at the point where every conversation needs to create a clear next step and protect momentum.';
    const messages={
      brief:`Hi ${first}, just a brief update on ${address}. We’re completing the first round of buyer follow-up from the latest open home and establishing each buyer’s position. ${weekLine}\n\nI’ll keep you posted as the conversations progress.`,
      full:`Hi ${first}, please see our full campaign feedback update for ${address}. We’ve now completed the next round of buyer follow-up and are working to separate genuine interest from buyers who are still comparing. ${weekLine}\n\nThe next step is to keep the strongest buyers moving and create a clear position before the weekend.`,
      pulse:`Hi ${first}, a quick midweek pulse on ${address}. We’ve reviewed the new enquiry and followed up the buyers requiring a next step. ${weekLine}\n\nWe’ll continue working the active conversations and keep you informed of any meaningful movement.`,
      progress:`Hi ${first}, a quick progress update on ${address}. We’re completing another round of buyer calls and pushing the engaged buyers for a clear next commitment. ${weekLine}\n\nThe goal is to create competition and have the campaign in the strongest possible position heading into the weekend.`,
      expectations:`Hi ${first}, just setting expectations ahead of Saturday for ${address}. We’ve reviewed the enquiry, followed up the buyers who may attend and confirmed the open-home plan. ${weekLine}\n\nI’ll update you after the open with attendance and the immediate follow-up plan.`,
      'post-ofi':`Hi ${first}, a quick update following today’s open home at ${address}. We’re now moving straight into buyer follow-up, confirming the strongest interest and establishing the next action with each attendee.\n\nI’ll keep you posted as those conversations unfold.`,
      weekly:`Hi ${first}, just closing out the week on ${address}. We’re reviewing the active buyer conversations, any incomplete follow-up and the strongest opportunity to progress next week.\n\nI’ll keep the campaign moving and update you on the next recommended step.`
    };return messages[plan.message]||messages.brief};
  function render(){const overview=$('#listingsOverview'),detail=$('#listingDetail');if(!overview||!detail)return;if(activeId&&find(activeId)){overview.classList.add('hidden');detail.classList.remove('hidden');renderDetail(find(activeId));return}activeId='';selectedActivityDate='';overview.classList.remove('hidden');detail.classList.add('hidden');renderOverview()}
  function renderOverview(){const list=all(),dueToday=list.filter(l=>isLogDay(l,todayKey())&&!isDone(l,todayKey())).length,overdue=list.reduce((n,l)=>n+overdueCount(l),0),complete=list.filter(l=>isLogDay(l,todayKey())&&isDone(l,todayKey())).length;
    $('#listingsPortfolioSummary').innerHTML=`<article><span>ACTIVE</span><strong>${list.length}</strong><small>campaign${list.length===1?'':'s'}</small></article><article><span>TODAY</span><strong>${dueToday}</strong><small>activities due</small></article><article><span>OVERDUE</span><strong>${overdue}</strong><small>actions to recover</small></article>`;
    $('#listingsGrid').innerHTML=list.length?list.map(l=>{const due=nextDueDate(l),plan=due?activityFor(l,due):null,score=execution(l),over=overdueCount(l);return `<button class="listing-card" type="button" data-open-listing="${esc(l.id)}"><div class="listing-card-top"><div class="listing-card-address"><span class="listing-card-status">${esc(l.status==='under-offer'?'Under offer':`Week ${campaignWeek(l)}`)}</span><strong>${esc(l.address)}</strong><small>${esc(l.vendorName||'Vendor not added')} · Campaign day ${daysBetween(l.startDate)}</small></div><div class="listing-health" style="--health:${score}"><strong>${score}%</strong></div></div><div class="listing-card-meta"><div><span>TODAY</span><strong>${isLogDay(l,todayKey())?(isDone(l,todayKey())?'Complete':'Due'):'No activity'}</strong></div><div><span>OVERDUE</span><strong>${over}</strong></div><div><span>STATUS</span><strong>${l.status==='under-offer'?'Under offer':'Live'}</strong></div></div><div class="listing-next-action"><div><span>NEXT CAMPAIGN ACTION</span><strong>${esc(plan?.title||'No activity scheduled')}</strong><small>${due?esc(formatShortDate(due)):''}</small></div><i>›</i></div></button>`}).join(''):`<section class="listings-empty"><strong>No active listings yet</strong><span>Add a campaign and AGNT will build the repeating activity calendar automatically.</span><button type="button" data-add-listing>Add your first listing</button></section>`;
  }
  function calendarDates(l){const start=mondayOf(addDays(todayKey(),-7));const dates=[];for(let i=0;i<21;i++)dates.push(addDays(start,i));return dates}
  function calendarHtml(l){return calendarDates(l).map(k=>{const plan=activityFor(l,k),scheduled=isLogDay(l,k)&&parseKey(k)>=parseKey(l.startDate),done=scheduled&&isDone(l,k),past=parseKey(k)<parseKey(todayKey()),today=k===todayKey(),selected=k===selectedActivityDate;let state=!scheduled?'off':done?'done':past?'overdue':today?'today':'upcoming';return `<button type="button" class="listing-calendar-day ${state} ${selected?'selected':''}" data-activity-date="${k}" ${!scheduled?'disabled':''}><span>${plan.short}</span><strong>${parseKey(k).getDate()}</strong><small>${!scheduled?'Off':done?'Done':past?'Due':today?'Today':`W${campaignWeek(l,k)}`}</small></button>`}).join('')}
  function chooseDefaultDate(l){if(selectedActivityDate&&isLogDay(l,selectedActivityDate))return selectedActivityDate;const due=nextDueDate(l);selectedActivityDate=due||scheduledDates(l,todayKey(),1)[0]||todayKey();return selectedActivityDate}
  function renderDetail(l){const date=chooseDefaultDate(l),plan=activityFor(l,date),entry=entryFor(l,date),context=contextFor(l,date),score=execution(l),status=statusCopy(l),week=campaignWeek(l,date),over=overdueCount(l);
    $('#listingDetail').innerHTML=`
      <header class="listing-detail-head"><button class="listing-back" type="button" data-listing-back aria-label="Back to listings">‹</button><div class="listing-detail-head-copy"><span class="listing-kicker">${l.status==='under-offer'?'UNDER OFFER':`CAMPAIGN WEEK ${campaignWeek(l)}`}</span><h2>${esc(l.address)}</h2></div><button class="listing-menu-button" type="button" data-edit-listing aria-label="Edit listing">•••</button></header>
      <section class="listing-health-hero glass"><div class="listing-health listing-health-large" style="--health:${score}"><strong>${score}%</strong></div><div class="listing-health-copy"><span>AGENT EXECUTION</span><h3>${esc(status[0])}</h3><p>${esc(status[1])}</p></div></section>
      <section class="listing-accountability-strip"><article><span>CAMPAIGN DAY</span><strong>${daysBetween(l.startDate)}</strong></article><article><span>THIS WEEK</span><strong>Week ${campaignWeek(l)}</strong></article><article><span>OVERDUE</span><strong>${over}</strong></article></section>
      <section class="listing-panel glass"><div class="listing-panel-head"><div><span>NEXT BEST ACTION</span><h3>${esc(plan.title)}</h3></div><em>${esc(formatShortDate(date))}</em></div><p class="listing-objective">${esc(plan.objective)}</p><div class="listing-context"><span>${esc(context.label)}</span><p>${esc(context.line)}</p></div></section>
      <section class="listing-panel glass"><div class="listing-panel-head"><div><span>CAMPAIGN CALENDAR</span><h3>Daily campaign rhythm</h3></div></div><div class="listing-calendar">${calendarHtml(l)}</div><p class="listing-calendar-note">Selected activity days repeat every week until the campaign is marked sold.</p></section>
      <section class="listing-panel glass"><div class="listing-panel-head"><div><span>${esc(formatLongDate(date).toUpperCase())}</span><h3>${esc(plan.title)}</h3></div><b>WEEK ${week}</b></div><div class="listing-daily-steps">${plan.steps.map((step,i)=>`<label><input type="checkbox" data-activity-step="${i}" ${entry.steps?.[i]?'checked':''}><span>${esc(step)}</span></label>`).join('')}</div><label class="listing-activity-note">Campaign note<textarea data-activity-note placeholder="Optional context for this day">${esc(entry.note||'')}</textarea></label><div class="listing-daily-actions"><button type="button" class="listing-message-button" data-listing-prompt="daily">Vendor message</button><button type="button" class="listing-complete-button ${entry.done?'done':''}" data-complete-activity>${entry.done?'Completed ✓':'Complete activity'}</button></div></section>
      <section class="listing-panel glass listing-support-panel"><div class="listing-panel-head"><div><span>VENDOR COMMUNICATION</span><h3>Support the campaign action</h3></div></div><p>Complete the campaign work first, then use the ready message as a starting point for the vendor group.</p><button type="button" data-listing-prompt="daily">Preview today’s message</button></section>`;
  }
  function openListingForm(existing){const l=existing||hydrate({}),overlay=document.createElement('div');overlay.className='listing-sheet-backdrop';overlay.innerHTML=`<section class="listing-sheet" role="dialog" aria-modal="true"><div class="listing-sheet-handle"></div><div class="listing-sheet-head"><div><span>${existing?'EDIT CAMPAIGN':'NEW CAMPAIGN'}</span><h2>${existing?'Campaign setup':'Add a listing'}</h2></div><button class="listing-sheet-close" type="button" data-sheet-close>×</button></div><form class="listing-form" id="listingEditorForm"><label>Property address<input name="address" required placeholder="23 Valencia Street, Toongabbie" value="${esc(l.address)}"></label><label>Vendor name(s)<input name="vendorName" placeholder="John & Mary" value="${esc(l.vendorName)}"></label><div class="listing-form-row"><label>Campaign start<input name="startDate" type="date" required value="${esc(l.startDate)}"></label><label>Method<select name="method"><option ${l.method==='Auction'?'selected':''}>Auction</option><option ${l.method==='Private Treaty'?'selected':''}>Private Treaty</option><option ${l.method==='Expressions of Interest'?'selected':''}>Expressions of Interest</option><option ${l.method==='Off Market'?'selected':''}>Off Market</option></select></label></div><div class="listing-form-row"><label>Auction date<input name="auctionDate" type="date" value="${esc(l.auctionDate)}"></label><label>Status<select name="status"><option value="live" ${l.status==='live'?'selected':''}>Live</option><option value="under-offer" ${l.status==='under-offer'?'selected':''}>Under offer</option><option value="sold" ${l.status==='sold'?'selected':''}>Sold</option></select></label></div><fieldset class="listing-day-selector"><legend>Days to log</legend><div>${[1,2,3,4,5,6,0].map(n=>`<label><input type="checkbox" name="logDays" value="${n}" ${l.logDays.includes(n)?'checked':''}><span>${DAY_PLAN[n].short}</span></label>`).join('')}</div></fieldset><label>Campaign notes<textarea name="notes" placeholder="Optional vendor drivers or campaign context">${esc(l.notes)}</textarea></label><button class="primary" type="submit">${existing?'Save changes':'Create campaign'}</button>${existing?'<button class="danger-link" type="button" data-delete-listing>Archive listing</button>':''}</form></section>`;document.body.append(overlay);const close=()=>overlay.remove();overlay.onclick=e=>{if(e.target===overlay||e.target.closest('[data-sheet-close]'))close();if(e.target.closest('[data-delete-listing]')){if(confirm('Archive this listing from AGNT?')){write(read().map(hydrate).filter(x=>x.id!==l.id));activeId='';close();render();toast('Listing archived')}}};overlay.querySelector('form').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),days=f.getAll('logDays').map(Number);if(!days.length){toast('Select at least one activity day');return}const saved=saveListing({...l,address:f.get('address'),vendorName:f.get('vendorName'),startDate:f.get('startDate'),method:f.get('method'),auctionDate:f.get('auctionDate'),status:f.get('status'),logDays:days,notes:f.get('notes')});activeId=saved.status==='sold'?'':saved.id;selectedActivityDate='';close();render();toast(saved.status==='sold'?'Campaign marked sold':existing?'Campaign updated':'Campaign added')};}
  function persistActivity(l,date,patch){l.activityLog={...l.activityLog,[date]:{...entryFor(l,date),...patch,updatedAt:Date.now()}};return saveListing(l)}
  function openPrompt(l,date){const text=messageFor(l,date),plan=activityFor(l,date),overlay=document.createElement('div');overlay.className='listing-sheet-backdrop';overlay.innerHTML=`<section class="listing-sheet"><div class="listing-sheet-handle"></div><div class="listing-sheet-head"><div><span>VENDOR MESSAGE</span><h2>${esc(plan.title)}</h2></div><button class="listing-sheet-close" type="button" data-sheet-close>×</button></div><p class="listing-share-note">WhatsApp will open with this message ready. Select the existing vendor group, review and send.</p><div class="listing-prompt-preview" contenteditable="true" id="listingPromptText">${esc(text)}</div><div class="listing-prompt-actions" style="margin-top:12px"><button class="copy" type="button" data-copy-prompt>Copy</button><button class="whatsapp" type="button" data-whatsapp-prompt>Share to WhatsApp</button></div><button class="listing-mark-sent" type="button" data-mark-message-sent>${entryFor(l,date).messageSent?'Marked sent ✓':'Mark message sent'}</button></section>`;document.body.append(overlay);const close=()=>overlay.remove();overlay.onclick=async e=>{if(e.target===overlay||e.target.closest('[data-sheet-close]'))close();if(e.target.closest('[data-copy-prompt]')){await navigator.clipboard?.writeText(overlay.querySelector('#listingPromptText').innerText);toast('Message copied')}if(e.target.closest('[data-whatsapp-prompt]')){const msg=overlay.querySelector('#listingPromptText').innerText;window.open(`https://wa.me/?text=${encodeURIComponent(msg)}`,'_blank')}if(e.target.closest('[data-mark-message-sent]')){persistActivity(l,date,{messageSent:true,messageSentAt:Date.now()});close();render();toast('Vendor message marked sent')}}}
  function bind(){
    $('#addListingButton')?.addEventListener('click',()=>openListingForm());$('#addListingTextButton')?.addEventListener('click',()=>openListingForm());
    $('#listingsView')?.addEventListener('click',e=>{const add=e.target.closest('[data-add-listing]');if(add)return openListingForm();const open=e.target.closest('[data-open-listing]');if(open){activeId=open.dataset.openListing;selectedActivityDate='';render();return}if(e.target.closest('[data-listing-back]')){activeId='';selectedActivityDate='';render();return}const l=activeId&&find(activeId);if(!l)return;if(e.target.closest('[data-edit-listing]'))return openListingForm(l);const dateButton=e.target.closest('[data-activity-date]');if(dateButton){selectedActivityDate=dateButton.dataset.activityDate;render();return}if(e.target.closest('[data-listing-prompt]'))return openPrompt(l,chooseDefaultDate(l));if(e.target.closest('[data-complete-activity]')){const date=chooseDefaultDate(l),entry=entryFor(l,date),steps=activityFor(l,date).steps.map((_,i)=>entry.steps?.[i]||false);persistActivity(l,date,{done:!entry.done,doneAt:!entry.done?Date.now():null,steps});render();toast(entry.done?'Activity reopened':'Campaign activity complete')}});
    $('#listingsView')?.addEventListener('change',e=>{if(!e.target.matches('[data-activity-step]'))return;const l=find(activeId);if(!l)return;const date=chooseDefaultDate(l),entry=entryFor(l,date),steps=activityFor(l,date).steps.map((_,i)=>Boolean(entry.steps?.[i]));steps[Number(e.target.dataset.activityStep)]=e.target.checked;persistActivity(l,date,{steps})});
    $('#listingsView')?.addEventListener('input',e=>{if(!e.target.matches('[data-activity-note]'))return;const l=find(activeId);if(!l)return;persistActivity(l,chooseDefaultDate(l),{note:e.target.value})});
  }
  bind();return{render,reset(){activeId='';selectedActivityDate='';render()}};
})();
