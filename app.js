...
function renderMarketPulse(){
  ...
  const events=normaliseMarketPulseEvents(marketPulseEvents).sort((a,b)=>{const matchDiff=marketMatches(b).length-marketMatches(a).length;return matchDiff||b.createdAt-a.createdAt}),matched=events.reduce((n,event)=>n+marketMatches(event).length,0);
...
function launchCampaignShortcut(){const payload=campaignPayload();if(!payload.users.length)return toast('No eligible recipients');if(!confirm(`Open SendHype with ${payload.users.length} personalised messages?`))return;campaignHistory.unshift({id:prospectId(),name:payload.name,count:payload.users.length,createdAt:Date.now()});saveLocal();renderCampaignHistory();const input=encodeURIComponent(JSON.stringify(payload));window.location.href=`shortcuts://run-shortcut?name=${encodeURIComponent('SendHype')}&input=text&text=${input}`}
...
function endProspectingSession({completeMarketSession=false}={}){
  ...
  const overlay=document.createElement('div');overlay.className='prospect-session-review-overlay';overlay.innerHTML=`<section class="prospect-session-review glass" role="dialog" aria-modal="true" aria-label="Session review"><span class="eyebrow">SESSION REVIEW</span><h2>Strong work.</h2><p>Here’s what you completed.</p><div class="prospect-session-review-grid"><div><strong>${stats.calls}</strong><span>Calls</span></div><div><strong>${Number(stats.sms)||0}</strong><span>SMS</span></div><div><strong>${stats.connects}</strong><span>Connects</span></div><div><strong>${stats.temperate}</strong><span>Warm / Hot</span></div><div><strong>${stats.appointments}</strong><span>Appointments</span></div></div><button class="primary" type="button" data-close-session-review>Done</button></section>`;document.body.append(overlay);overlay.querySelector('[data-close-session-review]').onclick=()=>overlay.remove();
}
